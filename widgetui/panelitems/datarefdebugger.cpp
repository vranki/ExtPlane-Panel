...
void DataRefDebugger::setDataRefName(QString name) {
    // Unsubscribe old
    if(_currentName != "" && _client->isDataRefSubscribed(_currentName)) _client->unsubscribeDataRefByName(_currentName); //TODO: there seems to be something wrong with unsubscribing...
    _currentName = name;
    _currentValue = "";
